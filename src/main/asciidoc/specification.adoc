...
The GeoAPI documentation completed by the annexes A (Conformance), B (Source Java Archives) and C (Source Python prototypes)
are normative, while the annexes D (Types and methods), E (UML diagram for referencing operation types),
F (Departures from ISO standards) and G (Comparison with legacy OGC specifications) are informative.
