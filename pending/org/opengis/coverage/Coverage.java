 ...
 *
 * @revisit Work not finished. We need to port methods from legacy OGC 01-004 if and when useful,
...
public interface Coverage {
     ...
     */
    @UML(identifier="Coordinate Reference System", obligation=MANDATORY, specification=ISO_19123)
    ...
    Collection<Extent> getDomainExtents();
     ...
     * collection equals that of the {@linkplain #getDomains domains} collection. In other words,
     * there is one instance of {@link AttributeValues} for each instance of {@link DomainObject}.
     * Usually, these are stored values that are accessed by the
     * {@link #evaluate(DirectPosition,Collection) evaluate} operation.
     *
     * In the case of a continuous coverage, there is a transfinite number of instances
     * of {@link AttributeValues} for each {@link DomainObject}. A few instances may be stored as
     ...
     *
     * @revisit The ISO returns type is {@code RecordType}, which is not yet defined in GeoAPI.
     */
     ...
     * overlapping geometric objects, where the geometric objects are either {@link DomainObject}s
     * or {@link ValueObject}s.
     */
     ...
     * {@link DomainObject}s in the domain of the coverage each paired with its record of feature
     * attribute values. In the case of an analytical coverage, the operation shall return the
     * empty set.
     *
     ...
     * {@link DomainObject}s that lie within the specified geometry and period.
     * If {@code s} is null, the operation shall return all <var>geometry</var>-<var>value</var>
     * pairs that contain {@link DomainObject}s within {@code t}. If the value of {@code t} is null,
     * the operation shall return all <var>geometry</var>-<var>value</var> pair that contain
     * {@link DomainObject}s within {@code s}. In the case of an analytical coverage, the
     ...
     * {@link DomainObject}s nearest to the direct position and their distances from the direction
     * position. The sequence shall be ordered by distance from the direct position, beginning with
     * the record containing the {@link DomainObject} nearest to the direct position. The length of
     * the sequence (the number of <var>geometry</var>-<var>value</var> pairs returned) shall be no
     * greater than the number specified by the parameter {@code limit}. The default shall be to
     * return a single <var>geometry</var>-<var>value</var> pair. The operation shall return a
     * warning if the last {@link DomainObject} in the sequence is at a distance from the direct
     * position equal to the distance of other {@link DomainObject}s that are not included in the
     * sequence. In the case of an analytical coverage, the operation shall return the empty set.
     * <p>
    ...
    /**
     * Returns a set of {@link DomainObject}s for the specified record of feature attribute values.
     * Normally, this method returns the set of {@link DomainObject}s in the {@linkplain Domain domain}
     * that are associated with values equal to those in the input record. However, the operation
     * may return other {@link DomainObject}s derived from those in the domain, as specified by the
     * application schema.
     * <p>
     * <B>Example:</B> The {@code evaluateInverse} operation could return a set of contours derived
     * from the feature attribute values associated with the {@link GridPoints} of a
     * {@link GridCoverage}.
     */
