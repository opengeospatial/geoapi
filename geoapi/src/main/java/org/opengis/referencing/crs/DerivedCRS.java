...
public interface DerivedCRS extends GeneralDerivedCRS {
    ...
    default Datum getDatum() {
        final CoordinateReferenceSystem crs = getBaseCRS();
        return (crs instanceof SingleCRS) ? ((SingleCRS) getBaseCRS()).getDatum() : null;
    }
    ...
    default DatumEnsemble<?> getDatumEnsemble() {
        final CoordinateReferenceSystem crs = getBaseCRS();
        return (crs instanceof SingleCRS) ? ((SingleCRS) getBaseCRS()).getDatumEnsemble() : null;
    }
}
